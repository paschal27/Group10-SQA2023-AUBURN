...
def checkIfWeirdYAML(yaml_script):
    logObj.info('checking checkIfWeirdYAML function. To make sure that correct file is gotten from directory')
    '''
...
def keyMiner(dic_, value):
    logObj.info('checking keyMiner function. To make sure that correct file is gotten from directory')
    '''
...
def checkIfValidHelm(path_script):
    val_ret = False
    if ((constants.HELM_KW in path_script) or (constants.CHART_KW in path_script) or (
            constants.SERVICE_KW in path_script) or (constants.INGRESS_KW in path_script) or (
                constants.HELM_DEPLOY_KW in path_script) or (constants.CONFIG_KW in path_script)) and (
            constants.VALUE_KW in path_script):
        val_ret = True
    return val_ret
