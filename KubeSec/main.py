...
def getCountFromAnalysis(ls_):
    logObj.info('checking getCountFromAnalysis function. To make sure that correct file is gotten from directory')
    list2ret = []
    for tup_ in ls_:
        within_sec_cnt = 0
        dir_name = tup_[0]
        ...
        list2ret.append((dir_name, script_name, within_sec_cnt, len(taint_secret), len(privilege_dic), len(http_dict),
                         len(secuContextDic), len(nSpaceDict), len(absentResoDict), len(rollUpdateDic),
                         len(netPolicyDict), len(pidfDict), len(ipcDict), len(dockersockDic), len(hostNetDict),
                         len(cap_sys_dic), len(host_alias_dic), len(allow_priv_dic), len(unconfined_dic),
                         len(cap_module_dic), k8s_flag, helm_flag))
    return list2ret
...
if __name__ == '__main__':
    ...
    ORG_DIR         = '/Users/pascal/Documents/tryy'
    OUTPUT_FILE_CSV = '/Users/pascal/Documents/tryy/V16_GITLAB_OUTPUT.csv'
