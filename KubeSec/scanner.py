...
def isValidUserName(uName):
    logObj.info('checking isValidUserName function. To make sure that correct file is gotten from directory')
    valid = True
        ...
        if (constants.PRIVI_KW in just_keys) and (constants.DEAMON_KW not in kind_values):
            ...
            for value_ in privilege_values:
                if value_ == True:
                    ...
                    if (constants.CONTAINER_KW in key_lis_holder) and (constants.SECU_CONT_KW in key_lis_holder) and (
                            constants.PRIVI_KW in key_lis_holder):
                        key_count += 1
...
def scanForResourceLimits(path_scrpt):
    ...
    if (parser.checkIfValidK8SYaml(path_scrpt)):
        ...
        if (constants.POD_KW in kind_entries):
            if ((constants.CONTAINER_KW in key_list) and (constants.LIMITS_KW not in key_list) and (
                    (constants.CPU_KW not in key_list) or (constants.MEMORY_KW not in key_list))):
                cnt += 1
                if (len(temp_ls) > 0):
                    all_values = list(parser.getValuesRecursively(yaml_di))
                    # print(all_values)
...
def scanForRollingUpdates(path_script):
    ...
    if (parser.checkIfValidK8SYaml(path_script)):
        ...
        '''
        key_list = [x_[0] for x_ in temp_ls]
        if ((constants.STRATEGY_KW not in key_list) and (constants.ROLLING_UPDATE_KW not in key_list) and (
...
def scanForMissingNetworkPolicy(path_script):
    ...
    if (parser.checkIfValidK8SYaml(path_script)):
        ...
        if (constants.NET_POLICY_KW not in all_values):
            ...
            '''
            key_list = [x_[0] for x_ in temp_ls]
            if ((constants.SPEC_KW in key_list) and (constants.POD_SELECTOR_KW in key_list) and (
                    constants.MATCH_LABEL_KW in key_list)):
    ...
    for yml_ in all_yml_files:
        ...
        if (parser.checkIfWeirdYAML(yml_) == False):
            if (parser.checkIfValidK8SYaml(yml_)) or (parser.checkIfValidHelm(yml_)):
                ...
                all_content.append((dir2scan, yml_, within_secret_, templ_secret_, valid_taint_secr, valid_taint_privi,
                                    http_dict, absentSecuContextDict, defaultNameSpaceDict, absentResourceDict,
                                    rollingUpdateDict, absentNetPolicyDic, pid_dic, ipc_dic, dockersock_dic,
                                    host_net_dic, cap_sys_dic, host_alias_dic, allow_privi_dic, unconfied_seccomp_dict,
                                    cap_module_dic, k8s_flag, helm_flag))
